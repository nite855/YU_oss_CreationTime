...
<img width="1248" height="543" alt="image" src="https://github.com/user-attachments/assets/9fef1c77-7dee-4af7-b7f9-775fa74157bd" />
...
이 장은 프로젝트에서 사용될 use case list를 간략하게 설명하였다. 사용자는 역할에 따라 가입을 안 한 사용자 Guest, 가입은 했지만 팀에 속하지 않은 RegisteredUser, 팀에 속한 TeamMember, 팀을 생성/관리하는 TeamLeader로 구분된다.
...
| Actor | Description |
|------|-------------|
| RegisteredUser, TeamMember, TeamLeader | 사용자는 ID와 비밀번호를 통해 인증을 수행하고 시스템에 로그인하며, 패널티 점수에 따라 접근이 제한될 수 있다. TeamLeader는 추가적으로 팀 관리 기능을 사용할 수 있다. |
...
| Actor | Description |
|------|-------------|
| TeamMember, TeamLeader | 다른 사용자가 작성한 글을 확인하여 팀 활동과 관련된 정보를 공유받고, 게시글 및 댓글을 통해 팀원 간 의사소통을 수행할 수 있다. |
...
| Approach | RegisteredUser, TeamMember, TeamLeader가 ID와 비밀번호를 입력하면 시스템은 입력된 정보를 기반으로 사용자 인증을 수행한다. 인증에 실패할 경우 오류 메시지를 출력하며, TeamMember, TeamLeader 혹은 RegisteredUser에게 부여된 패널티 점수에 따라 시스템 접근이 제한될 수 있다. |
| Dynamics | RegisteredUser, TeamMember, TeamLeader가 로그인 버튼을 누를 경우 |
...
| Goals | 모집 글을 작성하여 모집글 게시판에 글이 올라오고, 다른 RegisteredUser가 팀에 가입할 수 있게 된다. |
...
| Approach | RegisteredUser는 모집글을 확인하고 모집글 하단부에 있는 버튼을 눌러 해당 팀에 지원할 수 있다. |
...
| 항목 | 내용 |
|------|------|
| Purpose | TeamMember, TeamLeader가 게시글을 작성하여 팀원 간 정보 공유 및 공지를 남길 수 있도록 한다. |
| Approach | TeamMember, TeamLeader는 게시글을 작성하고 시스템은 이를 저장하여 다른 팀원이 확인할 수 있도록 한다. |
| Dynamics | TeamMember, TeamLeader가 게시글을 작성할 경우 |
...
| 항목 | 내용 |
|------|------|
| Purpose | TeamMember, TeamLeader가 다른 사용자가 올린 글을 확인하고, 의사소통을 진행할 수 있도록 한다. |
| Approach | TeamMember, TeamLeader는 게시글을 조회하고, 게시글에 댓글을 작성하여 의견을 공유할 수 있다. |
| Dynamics | TeamMember, TeamLeader가 게시글 목록에서 게시글을 선택할 경우 |
| Goals | TeamMember, TeamLeader가 팀 활동 진행에 대한 정보를 얻거나 TeamLeader의 공지를 확인할 수 있다. 글 하단부에 댓글을 작성하여 의사소통을 진행할 수 있다. |
...
| TeamMember | Team 내에서 투표, 게시글 작성, 일정 확인 등 팀 활동에 참여하는 주체 |
| TeamLeader | Team을 생성하고 관리하는 권한을 가진 주체 |
